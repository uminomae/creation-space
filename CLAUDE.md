...
- **🚫 原典重複禁止（FAIL 不変条件・Check 10, cs#249）**: 同一原典（著者+年+書名）を同一領域に複数行登録してはならない。重複は 1:1 原則違反。manifest に行を追加する前に必ず `grep` で既存の同一原典を確認すること。版違い・クロス領域 anchor は §1/§7 の正規パターン（領域が異なる、またはレビュー済例外のみ許可）。**重複と気づいたら注記して放置せず、その場で重複行を除外する**（D15-S10/D26-S09 は「重複」注記しながら放置されていた = 再発させてはならない QC 欠陥）
