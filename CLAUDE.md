...
- **🔗 鎖の不変条件（cs#252・Check 11）**: 公開出力の鎖「持つ(誰でも検証可)→読む→解釈→まとめる→公開」。取得不能（citation-only/blocked-access）な原典の上に公開解釈/確定論拠を置いてはならない（11a: source-note 禁止 / 11b: 取得不能を取得不能な代替で確定するな）。残せるのは read-list（書誌＋読みたい理由）のみ
- 検知: `.claude/hooks/source-note-gen-check.sh` (SessionStart) / `source-note-gen-notify.sh` (commit) / `validate-manifest-sync.sh` Check 10 (重複) / Check 11 (鎖: 取得不能に公開解釈/確定論拠を置かない) / Check 12 (鎖の「読む」: abstract/メタデータのみの source-note 禁止)
