...
過去の調査（Phase 1-7）は **LLM に質問して得た回答をベースに分析・検証**を行い、いったん完了とした。しかし**原典（論文本文）を実際に読まずに**行っていたため、pjdhiro の判断で**原典精読ベースへ全面的に方針変更**した。ほぼ一からやり直しである。

### 新しい調査パイプライン

1. **原典を探す** — OA URL 探索、PDF 取得
2. **原典を読む** — 本文を精読し、構造類似性を原典ベースで判定する
3. **wiki にまとめる** — 読んだ内容を公開可能な形で整理する
4. **原典と wiki を参照しながら構造類似の分析を行う**

### 現在地（2026-04-10）

- evidence-D*.md は全て stub（旧本文は `archive/pre-rerun-20260407/` に退避）
- 原典探索: 435本追跡中、270本アクセス達成 (62%)
- 類似論文: 153本（VAL 検証済み）
- **次のステップ**: 原典の精読→wiki化→原典ベースの構造類似分析
- 追跡 Issue: cs#217（調査体系レビュー）

## 目的

30領域の構造類似調査で得られた evidence、一次ソース、深掘り探索、レビュー素材を格納する。
