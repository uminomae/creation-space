...
- **H08.bs（構造・履歴依存）**：  
  「信頼軸へのルーティングが起きやすい」という**長期の基礎条件**。  
  *候補神経基盤*: vmPFC–扁桃体系の**構造的結線**（白質・結線強度）／発達史により形成。
- **H08.pl（活動・状況依存）**：  
  その場で「信頼軸へ流す／流せる」ための**短期の起動状態**。  
...
| 反応時間・行動 | 情動干渉課題（例：emotion Stroop）、信頼ゲームでの投資額/更新率、協力課題での修復行動 | 「信頼軸へ回す」行動痕跡 | [P] |
...
2. **H08.plは単なる気分ではなく「ルーティング状態（gating）」**  
   stateは感情状態・主観状態を指しがちだが、H08.plは「誤差が信頼軸へ流れる／流せる」**計算過程の作動状態**に近い。  
