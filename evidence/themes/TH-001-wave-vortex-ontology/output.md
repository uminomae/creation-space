...
**中心の問い（発端 2026-07-09 の Kandyba 記事に由来する二段構造）**: pjdhiro の直観「宇宙が海の渦のような現象でできている」「創造とは波間の渦のよう」には二つの深さがある——(i) 時空の**中身**（素粒子・秩序・生命）が波間の渦のようか、(ii) **器そのもの**（時空それ自体）までもがそうか。調査の最終回答（2026-07-11 確定）: **中身は複数分野の一次資料で強く支持される（§3 三方面収束・§1 数学的対応）。器は開いた問いのまま（§2: 正面から主張する理論はあるが LIV 関門を通過した理論は未発見）**。この器/中身の区別自体が、超流体 DM が関門を回避できる理由の説明鍵になった（§2 W3）。

本文書は、この直観を実証の厚みが異なる三つの層に分けて接地する。各層は信頼度が違い、混同すると誤読を生む。そこで層ごとに信頼度を宣言し、層をまたぐ言及には [SCALE_JUMP] または [M]/[S] を付す。
...
Steinhauer らの BEC 音響ブラックホール系列は、この分野が一度の測定で確定せず、反証と改良を経て確立することを示す好例である。2016 年の自発ホーキング放射報告（Nature Physics）は Leonhardt の再解析で統計的アーティファクトの可能性を指摘された（arXiv:1609.03803）。チームは装置に「多数の改良」（低ノイズ磁場環境・光学系の再設計・参照画像法等。本文の表現は "many improvements"）を加え、2019 年（Muñoz de Nova et al., Nature 569, 688 = arXiv:1809.00913、**本文確認済み**）で実験 7,400 回の反復から予測ホーキング温度 0.35 nK（0.12 mc²_out）と熱的スペクトルの一致を報告、放射がブラックホール内側では負エネルギーパートナーのみで構成されアナログファイアウォールが無いことも示した。2021 年（Kolobov et al., Nature Physics 17, 362 = arXiv:1910.09363 CC-BY、**本文確認済み**）は実験 97,000 回＝連続 124 日分の測定により、自発放射の**定常性**を 6 時点で確認し、内側地平線形成後の刺激放射が black-hole lasing でなく Bogoliubov-Cherenkov-Landau 刺激で説明されることを示した [P]。Leonhardt は 2019 年の結果について「ジェフの仕事を心から祝福する。研究コミュニティにとって重要な一歩だ（I really congratulate Jeff on his work, which is an important step for the community）」と公に評価を転じた（Physics World 2019 の直接引用。二次報道 [M]。旧稿の引用「これは私が信じ始めてもよい最初のものだ」は同記事・Nature news のいずれにも見つからず、R2 全文照合 2026-07-11 で検証可能な引用に差し替え）。

> **訂正（W2, 2026-07-09）**: 旧稿の「チームは 21 回の改良を重ね」という数字は 2019 年論文本文に存在しない（本文は "many improvements" とのみ記す）。survey 由来の未検証数字だったため撤回する。
> **出所判明（R2, 2026-07-11）**: この「21」は Physics World 2019 記事の "Steinhauer's team made 21 improvements to the experiment" に由来と判明（ハルシネーションではなく二次報道由来の数字）。論文本文にない数字である以上、本文非掲載の判断は維持する。

Švančara et al. 2024（Nature 628, 66-70、PMC 本文確認済み）は、超流動ヘリウム4中に約 10^4 個の循環量子を単一コアに閉じ込めた定常巨大量子渦を安定化した。共回転モード（m=+8）で4つの離散ボウンドステートを測定し、反回転モード（m=-8）では、先行研究でリングダウンモードと同定されていた種類の励起を記録し、論文はこれを「この過程が量子流体で起きている最初の兆候（the very first hints of this process taking place in a quantum fluid）」と表現した（R2 照合 2026-07-11: 旧稿の「初の証拠」は原文より強い表現だったため原文どおりに修正）。ここは一組で運ぶ必要がある。すなわち「論文が命名した励起を観測した」ことは実データ [P] だが、同時に論文自身が「エルゴ領域の直接観測は未達成」と明記しており、「リングダウン」という命名と実カー計量の準固有モードとの数理的対応の厳密さは本文主文からは判定しきれず保留（Methods 未確認）である。したがってこの言明を「カー計量が実在した」へ格上げすることはできない [M寄りP]。
...
三つは分野も方法も独立でありながら、同じ「形は過程である」という描像に収束している。

**W8（2026-07-10）— 三方面収束の独立性を敵対的パネルで検討し、知の歴史として位置づけ直した**。争点「三方面は真に独立か」を adversarial-review スキル（cs#260 初回試行）で扱った。パネル構成は正直に開示する: 外部 CLI（別ファミリー）が当環境で不可のため、階層②別 Claude モデル（Opus/Sonnet）＋階層③方法論分岐（系譜追跡／反証専任）に**縮退**。同一ファミリー収束のため証拠力は格下げして扱うが、両パネリストは**検証可能な史実**に収束した（vibes でなく引用・謝辞での照合）ため、結論の骨子は堅い。
...
2. **Bohm 川の渦比喩の一次確認**: → **解決（W1, 2026-07-09）**。原著（Routledge 1980）pp.12-13, 24, 38 で一次確認。**全文通読での再照合も完了（R3, 2026-07-11）——追加の不一致なし**。§3 参照。
