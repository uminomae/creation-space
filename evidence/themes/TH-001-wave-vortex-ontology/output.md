...
| 2 | プリゴジン散逸構造 | 渦→束 | エネルギー流下で秩序が維持される。cs guide「維持構造型の束」と構造類似 | [M] | **補強**（guide 既採用対応の再確認） |
| 3 | シモンドン個体化 | 縁→渦 | 準安定な前個体的場からの transduction で個が立ち上がる（例は結晶で渦でない） | [M] | **部分的当てはめ**（構造対応は立つが渦性は借用） |
...
**この枠組み自身の限界と、最強の将来テスト**: 本枠組みは対応づけを行った調査者（Claude）自身による自己検査であり、確証バイアスを完全には除けない。最強の外部テストは**盲検再導出**——五段階モデルの定義と源流テキストだけを与えられた（対応表を見ていない）独立の読み手が、同じ対応を再構成するか。再構成されれば「発見」の強い証拠、されなければ「当てはめ」の兆候。これは文脈ゼロ subagent を要するため、枠回復後の実行課題として PLAN に登録する（W4b）。
...
3. **「発見」か「事後的当てはめ」か**: → **枠組み確立（W4, 2026-07-10）**。5テスト・三値判定の枠組みを §4.1 に明文化し、接地候補5件へ適用済み（発見寄り1・補強1・部分的当てはめ1・当てはめ2）。候補4の循環は「モデル独立の史実／追加された読み」の2層分解で解消。**残余（W4b）**: 盲検再導出テスト（文脈ゼロ subagent 必須、枠回復後）。
