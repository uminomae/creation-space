...
- **supports**: M1-M4（4モジュール構造の外部的妥当性）, D4（F-O評価と動機）
- **refs**:
...
- [独自] フロー/創造の内側を4層（L0-L3）で機構記述する（欠損/抱持/F-O）
...
- **5段階対応**: 場←領域知識の蓄積, 波←課題動機による探索開始, 縁←創造性関連プロセス（発散・探索）, 渦←アイデアの具体化, 束←産物の評価。動機の質（F/O軸）が全段階を貫通
- **独自性**: 動機をF-O 2軸に分解し、創造性阻害を「F軸支配の失敗モード」として記述
- **牽強付会リスク**: 中。外発的動機が必ずしも阻害しない条件（情報的報酬）の扱いが未整理
- **supports**: D4（F-O評価と動機の質）, M2
- **refs**:
...
- **結論**: 保留（ISS-32起票）
- **信頼度**: 0.70
- **議論経緯**: 外発的動機が必ずしも阻害しない条件を検討。情報的報酬（能力フィードバック）vs 統制的報酬（行動制御）の区別がF-O評価と接続可能。統制的報酬=F軸優位→抱持困難、情報的報酬=O軸的→抱持維持可能という仮説を立てた。組織設計（M3）、教育への応用に直結する重要論点。
- **仮説**: 統制的報酬=F軸優位→抱持困難、情報的報酬=O軸的→抱持維持可能
- **論点**: 外発的動機が阻害しない条件
...
- **独自性**: 5段階を集団スケールに適用。集団抱持とF-O軸（安全/関係）で下支え
...
- **独自性**: 分解の作法として有用。4Pを内部機構（4層/F-O/抱持）へ接続する
...
- [独自] 4Pを"内部機構（4層/F-O/抱持）"へ接続し、Processを説明可能にする
...
- **独自性**: 価値化をF-O評価として内側からも記述し、投資の継続を抱持で支える
...
- [独自] 価値化をF-O評価として内側からも記述し、投資の継続を抱持で支える
...
- **独自性**: 表象変化を欠損（D2）と抱持（D3）の循環として説明し、情動（F-O）を組み込む
...
- [独自] 表象変化を、欠損（D2）と抱持（D3）の循環として説明し、情動（F-O）を組み込む
...
- [独自] ネットワーク協調を、欠損（D2）→F-O評価（D4）→抱持（D3）の層間循環に位置づける
...
|-------------|------------------------------|---------|
| 段階の内部構造が不明 | 5段階 + 4層で内部機構（欠損/抱持/F-O）を記述 | D1-D4, EV-NS-* |
