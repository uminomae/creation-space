...
2. **生存-信頼軸（D4）・抱持（D3）・Container/安全基地（EV-PA）との接続**
   - 信頼を「信頼軸の中心語」として使うなら、TC-009（感情ラベル分解）に従い、F/Oの評価・統合・破綻と接続できる形で書く。
...
- **ISS-42（最小単位）**: 間主観性の生存-信頼最小構成（S23の片側性を理論化する足場）
...
graph TD
  ...
  ISS42["ISS-42: 間主観性の生存-信頼最小構成"]:::hot
...
- D4（生存-信頼評価）: `base/schema/core-definitions.md`
...
- **B1: 信頼の「最小構成」候補を3案に分岐して提示**
  - 条件: 双方向性前提にしない／生存-信頼分解（TC-009）を破らない／D3-a（成立条件）と矛盾しない
...
- **C2: 片側信頼の類型（S23）→最小構成（ISS-42）への写像**
  - 出力: 母子/ブランド/神秘体験などの例を、生存-信頼最小構成で **何が共通で、何が違うか** として整理
...
- 信頼モデル（補助モデルとして開始）:
  ...
  - 含める内容: 最小構成、生存-信頼/抱持/Container接続、BSPLとの関係（ISS-41判定条件）
- 判断ログ（採否・保留理由）:
...
2. **感情ラベル“信頼”を使う箇所は生存-信頼分解できる**（TC-009）
