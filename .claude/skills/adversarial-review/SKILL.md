...
triggers: |
  ...
  「複数モデルで検証」「パネルレビュー」
status: draft (cs#260 — 形態・正本の置き場は pjdhiro 判断待ち。試行用ドラフト)
agent: "CLI (Main = ファシリテータ)"
---

# adversarial-review SKILL（ドラフト v0.1）
...
- [ ] 形態: 独立スキル vs agent-team-workflow REVIEW フェーズ強化（本ドラフトは独立スキル形式で作成。REVIEW 統合なら §3 を V3 の置換として移植）
- [ ] 正本の置き場: pd 集約ルールに従えば pd 正本＋cs ポインタ。本ファイルは cs 試行用ドラフト
- [ ] 外部 CLI（Codex）の標準パネリスト化の可否（コスト・認証の運用）
