...
> ⚠️ **本ディレクトリの成果物は生成日時点のスナップショット**（CL-010）。backfill（cs#249）や manifest 修正で前提は日単位で変わる。tier・has_note・STUB 判定等を根拠に行動する前に、**実物（source-note ファイル・manifest 行）を必ず再確認する**こと。実例: confidence-tier.jsonl は D13-S02 を `has_note: false` と記録していたが、生成6日後の参照時には全章精読ノート（209行）が既に存在していた。
